...
public class Main
{
            ...
            Map<String, Integer> goldData)
    {
        ...
        System.out.println("counts <- table(c(" + counts.stream().map(Object::toString)
                .collect(Collectors.joining(", ")) + "))");
        System.out.println(
                "barplot(counts, xlab=\"Number of systems that solved a particular instance\", ylab=\"Correct instances\", main=\"Distribution of correctly classified instances among all systems\", cex.lab=1.3, cex.main=1.3, las=2)");
            ...
            throws Exception
    {
        ...
        */
        ...
        List<Participant> participantsWithoutBaseline = participantsForComparison.stream()
                .filter(p -> !p.isBaseline()).collect(Collectors.toList());

        problematicInstanceDistribution(participantsWithoutBaseline, goldData);
    }
